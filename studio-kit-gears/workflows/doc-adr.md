...
It binds the ADR artifact KIND and the gears kit resources (template, rules,
checklist, example), injects ADR-specific authoring rules, and delegates the full
...
RULES:
  ALWAYS bind ARTIFACT_KIND = ADR and the four gears ADR references (template, rules, checklist, example) before delegating to cf-write-docs
  ALWAYS inject {adr_rules} as additional gears ADR authoring rules into every author dispatch
...
NOTES:
  cf-write-docs already drives the author -> deterministic gate (cfs validate --artifact) -> semantic review (cf-semantic-reviewer-artifact) loop; this preset only supplies the gears ADR KIND binding and ADR-specific rules.
```

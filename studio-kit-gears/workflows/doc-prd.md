...
It binds the PRD artifact KIND and the gears kit resources (template, rules,
checklist, example), injects PRD-specific authoring rules, and delegates the full
author -> deterministic-gate -> semantic-review loop to `cf-write-docs`. It
authors no content itself.
...
RULES:
  ALWAYS bind ARTIFACT_KIND = PRD and the four gears PRD references (template, rules, checklist, example) before delegating to cf-write-docs
  ALWAYS inject {prd_rules} as additional gears PRD authoring rules into every author dispatch
...
NOTES:
  cf-write-docs already drives the author -> deterministic gate (cfs validate --artifact) -> semantic review (cf-semantic-reviewer-artifact) loop; this preset only supplies the gears PRD KIND binding and PRD-specific rules.
```
